...
open class ImageCache :NSObject, NSSecureCoding {
	...
	fileprivate(set) lazy var cacheUrl: URL =
		{
			...
			do {
				...
				os_log("got error creating image cache direcctory: %@", type:.error, error)
	...
	func imageWithId(_ imageId:Int) -> PlatformImage? {
		...
		cache.setObject(NSData(data: imgData) as Data as Data as AnyObject, forKey: imageId as AnyObject)
