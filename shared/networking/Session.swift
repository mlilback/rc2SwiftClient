...
public protocol SessionMessageType: CVarArg {}

extension String: SessionMessageType {}
extension Data: SessionMessageType {}
...
open class Session : NSObject, SessionFileHandlerDelegate {
	...
	fileprivate var keepAliveTimer:DispatchSourceTimer = DispatchSource.makeTimerSource(flags: DispatchSource.TimerFlags(), queue: DispatchQueue.main)
	...
	init(_ wspace:Workspace, source:WebSocketSource, restServer rserver:RestServer, appStatus:AppStatus, networkConfig config:URLSessionConfiguration, hostIdentifier:String, delegate:SessionDelegate?=nil)
	{
		workspace = wspace
		...
		wsSource.event.message = { [unowned self] message in
			self.handleReceivedMessage(message as! SessionMessageType)
		}
...
private extension Session {
	...
	func processBinaryResponse(_ data:Data) {
		...
		do {
			...
			os_log("error parsing binary message:%@", type:.error, err as NSError)
		}
		...
		guard let message:String = dict["msg"] as? String else {
			os_log("got binary response w/o a message", type:.error)
			return
		}
		switch message {
		case "saveResponse":
		...
		default:
			os_log("received unknown binary message: %@", dict["msg"] as! String)
			return
		}
	}
	...
	func handleSaveResponse(_ rawDict:[String:AnyObject]) {
		...
		if let errorDict = rawDict["error"] as? Dictionary<String,AnyObject> {
			//TODO: inform user
			os_log("got save response error: %@", type:.error, (errorDict["message"] as? String)!)
			return
		}
		do {
			...
			guard let idx = workspace.indexOfFilePassingTest({ $0.fileId == file.fileId }) else {
				os_log("saveResponse for file not in workspace: %@", type:.error, file.name)
				return
			}
			workspace.replaceFile(at:idx, withFile: file)
		} catch let err as NSError {
			os_log("error parsing binary message: %@", type:.error, err)
		}
	}
	...
	func handleReceivedMessage(_ message:SessionMessageType) {
		if let stringMessage = message as? String {
			let jsonMessage = JSON.parse(stringMessage)
			os_log("got message %@", jsonMessage["msg"].stringValue)
			...
			os_log("invalid binary data format received: %@", type:.error, message)
		}
	}
	...
	@discardableResult func sendMessage(_ message:Dictionary<String,AnyObject>) -> Bool {
		...
		do {
			...
			os_log("error sending json message on websocket: %@", type:.error, err)
			return false
		}
		return true
	}
