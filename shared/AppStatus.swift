...
extension AppStatus {
	...
	func presentAlert(_ session:Session, message:String, details:String, handler:((Int) -> Void)? = nil) {
		presentAlert(session, message:message, details:details, buttons:[], defaultButtonIndex:0, isCritical:false, handler:handler)
	}

	///prsents a NSAlert/UIAlert with an optional callback
	func presentAlert(_ session:Session, message:String, details:String, buttons:[String], handler:((Int) -> Void)? = nil) {
	...
	///prsents a NSAlert/UIAlert with an optional callback
	func presentAlert(_ session:Session, message:String, details:String, buttons:[String], defaultButtonIndex:Int, handler:((Int) -> Void)? = nil) {
