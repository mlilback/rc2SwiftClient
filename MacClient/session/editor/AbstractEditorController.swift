...
class AbstractEditorController: AbstractSessionViewController, MacCodeEditor {
	...
	@objc func autosaveCurrentDocument() {
		...
		guard doc.isDirty, !self.appStatus!.busy else { autoSaveDisposable.value = nil; return }
	...
	func documentChanged(newDocument: EditorDocument?) {
		...
		if document.isLoaded {
			compositeDisposable += document.editedContents.signal.observe(on: UIScheduler()).observeValues { [weak self] _ in
