...
//	weak var fileDelegate:SessionFileHandlerDelegate?
	...
	init(workspace wspace: Workspace, baseUrl: URL, config: URLSessionConfiguration) {
		self.workspace = wspace
		self.fileCache = DefaultFileCache(workspace: workspace, baseUrl: baseUrl, config: config)
		self.baseUrl = baseUrl
		self.saveQueue = DispatchQueue(label: "fileHandlerSerial", attributes: [])
	}
	
	public func loadFiles() -> SignalProducer<SessionFileHandler, NSError> {
